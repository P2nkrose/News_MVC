package com.example.news_restapi.controller;

import com.example.news_restapi.model.News;
import com.example.news_restapi.model.NewsDAO;
...
public class NewsController extends HttpServlet {
    private NewsDAO dao;
    private ServletContext ctx; // 서버 로그 메세지 생성을 위해 선언.

    private final String START_PAGE = "resources/newsList.jsp"; // 웹 리소스 기본 경로 지정
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        /* 1. 자바 리플렉션으로 action으로 전달된 이름의 메서드를 자동으로 호출
            * 자바 리플렉션 API
            자바 클래스 구조를 프로그램에서 직접 제어할 수 있는 라이브러리로,
            구체적인 클래스 타입을 모르더라도 문자열 이름으로 클래스를 참조하거나
            클래스의 메서드나 변수를 읽어와 실행하는 등의 작업이 가능.
         */
        try {
            // 현재 클레스에서 action 이름과 HttpServletRequest를파라미터로 하는 메소드 찾음
            ...
            e.printStackTrace();
            ctx.log("요청 action 없음!!"); // 톰켓의 콘솔에 로드 메세지를 남기는 메소드
    ...
    public String getFilename(Part part) {
        ...
        // 파일 이름이 들어있는 속성 부분의 시작위치를 가져와 쌍따옴표 값 부분만 가지고옴
    ...
    public String addNews(HttpServletRequest request) {
        ...
        try {
            ...
            if (fileName != null && !fileName.isEmpty()) {
                part.write(fileName); // 파일 이름이 있으면 저장
            }
